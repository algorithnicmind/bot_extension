    ...
    btnSummarize.addEventListener('click', async () => {
        ...
        try {
            ...
            if (!tab || tab.url.startsWith('chrome://')) {
                hideLoaderAndShowError("Cannot run on Chrome system pages or settings.");
                return;
            }
            ...
            if (!pageText || pageText.length < 50) {
                 hideLoaderAndShowError("Not enough text found on this page to summarize.");
                 return;
            }
            ...
            chrome.runtime.sendMessage(
                ...
                (response) => {
                    if (chrome.runtime.lastError) {
                        hideLoaderAndShowError("Background script error: " + chrome.runtime.lastError.message);
                        return;
                    }
                    if (response && response.error) {
                        ...
                        displayResult(response.result);
                         ...
                         hideLoaderAndShowError("Unknown error occurred while processing AI response.");
                    }
                }
            );
