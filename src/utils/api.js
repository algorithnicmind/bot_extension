/**
 * API Utility for AI Assistant
 */

export async function handleAIRequest(provider, apiKey, contextText, type, userPrompt = null) {
  ...
  switch (type) {
    ...
    case "explain":
      ...
      break;
    case "translate":
      systemInstruction = "You are a master translator. Detect the original language of the provided text, and accurately translate the core content into English. If it is already in English, translate it to Spanish. Preserve the original meaning and tone.";
      break;
    case "explain_page":
      systemInstruction = "You are an expert tutor. Formulate a list of the key concepts and jargon present in this text, and explain each of them clearly.";
      break;
    case "chat":
      systemInstruction = "You are an AI sidekick attached to the user's browser. The user has provided the text of the current webpage they are looking at as context. Answer the user's question directly based on the provided text context. Do not invent new facts. If the answer is not in the context, tell the user.";
      break;
    default:
      systemInstruction = "You are a helpful AI assistant. Answer the user's question clearly and concisely based on the context provided.";
  }
  ...
  if (provider === "gemini") {
    ...
    throw new Error("Unsupported AI Provider Selected. Please check Options page.");
  }
}
